...
from lib.rel_model_prior import RelModelPrior
...
teacher1 = RelModelPrior(classes=train.ind_to_classes, rel_classes=train.ind_to_predicates,
                    num_gpus=conf.num_gpus, mode=conf.mode, require_overlap_det=True,
                    use_resnet=conf.use_resnet, order=conf.order,
                    nl_edge=conf.nl_edge, nl_obj=conf.nl_obj, hidden_dim=conf.hidden_dim,
                    use_proposals=conf.use_proposals,
...
for n, param in teacher1.named_parameters():
    param.requires_grad = False
print("Loading teacher parameters")
tckpt = torch.load(conf.teacher1_ckpt)
if conf.teacher1_ckpt.split('-')[-2].split('/')[-1] == 'vgrel':
    # import pdb; pdb.set_trace()
    print("Loading EVERYTHING")
    start_epoch = tckpt['epoch']
    if not optimistic_restore(teacher1, tckpt['state_dict']):
        start_epoch = -1
        # optimistic_restore(teacher1.detector, torch.load('checkpoints/vgdet/vg-28.tar')['state_dict'])
else:
    start_epoch = -1
    optimistic_restore(teacher1.detector, tckpt['state_dict'])
    teacher1.roi_fmap[1][0].weight.data.copy_(tckpt['state_dict']['roi_fmap.0.weight'])
    teacher1.roi_fmap[1][3].weight.data.copy_(tckpt['state_dict']['roi_fmap.3.weight'])
    teacher1.roi_fmap[1][0].bias.data.copy_(tckpt['state_dict']['roi_fmap.0.bias'])
    teacher1.roi_fmap[1][3].bias.data.copy_(tckpt['state_dict']['roi_fmap.3.bias'])
    teacher1.roi_fmap_obj[0].weight.data.copy_(tckpt['state_dict']['roi_fmap.0.weight'])
    teacher1.roi_fmap_obj[3].weight.data.copy_(tckpt['state_dict']['roi_fmap.3.weight'])
    teacher1.roi_fmap_obj[0].bias.data.copy_(tckpt['state_dict']['roi_fmap.0.bias'])
    teacher1.roi_fmap_obj[3].bias.data.copy_(tckpt['state_dict']['roi_fmap.3.bias'])
teacher1.cuda()
...
def soft_cross_entropy(inp, target):
    '''
    :param inp:
        logits (unadjusted for base error)
        [batch, C]
    :param target: multinomial over the classes 0 .. C-1
        [batch, C]
    :return:
    ...
    ts = inp*target
    return -torch.mean(torch.sum(ts, dim=1), dim=0)
...
print("Loading student checkpoints")
